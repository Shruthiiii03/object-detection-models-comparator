...
TREX_JSON_DIR = "TREX_predictions"
...
#build filepaths for dino, gemini, TRex

for base_name in all_filenames:
    xml_path = os.path.join(XML_DIR, f"{base_name}.xml")
    # json_path = os.path.join(JSON_DIR, f"{base_name}.json")
    pred_path = os.path.join(TREX_JSON_DIR, f"{base_name}.json")
    ...
    if not xml_exists and not pred_exists:
        ...
        pred_boxes = parse_json(pred_path)
        ...
        avg_iou, TP, FP, FN = evaluate_iou(xml_path, pred_path)
        total_tp += TP
        total_fp += FP
        total_fn += FN
...
#     xml_path = os.path.join(XML_DIR, f"{base_name}.xml")
#     # json_path = os.path.join(JSON_DIR, f"{base_name}.json")
#     pred_path = os.path.join(TREX_JSON_DIR, f"{base_name}.png")
#     image_path = os.path.join(IMG_DIR, f"{base_name}.png") 
